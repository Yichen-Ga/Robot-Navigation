...
#deleted the for loop wed 14:19 #uncommented
        ...
        for i in mapdata.data:  
            if (PathPlanner.is_cell_walkable(mapdata,x,y) == False):
                ...
                for X_frontier_padding in range(-frontier_padding, frontier_padding):
                    for Y_frontier_padding in range(-frontier_padding, frontier_padding):
                          if x+X_frontier_padding in range(frontier_x - 100 ,frontier_x + 100) and y+Y_frontier_padding in range( frontier_y - 100 , frontier_y + 100):
...
#=================================================
        ...
        if len(ListOfFrontiers) <= self.Frontier_value:
            #need to be a world coordinate
            target_frontier = Point()
            target_frontier.x = self.start.pose.position.x
            target_frontier.y = self.start.pose.position.y
