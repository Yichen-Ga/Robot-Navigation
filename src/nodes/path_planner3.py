...
from geometry_msgs.msg import Point, Pose, PoseStamped, Quaternion, PoseArray, PoseWithCovarianceStamped
...
class PathPlanner:
    ...
    def __init__(self):
        ...
        ## include global_localization service for amcl
        #self.global_local = rospy.Service("global_localization",Empty)
        ...
        rospy.Subscriber('/particlecloud',PoseArray, self.new_odometry)
    ...
    def new_odometry(self, msg):
        """
        Updates the current pose of the robot.
        This method is a callback bound to a Subscriber (The particle cloud we got from global localization)
        :param msg [PoseArray] is an array of Pose
        We need to get the average of x,y,and orientation of the Poses
        """
        self.starting_Position = PoseStamped()
        ...
        X_all_value = 0.0
        Y_all_value = 0.0
        Orientation_z_all_value = 0.0
        Orientation_w_all_value = 0.0

        # print(msg.poses[3])

        for i in range(0, len(msg.poses)):
            curr_pose = msg.poses[i]
            X_all_value += curr_pose.position.x
            Y_all_value += curr_pose.position.y
            Orientation_z_all_value += curr_pose.orientation.z
            Orientation_w_all_value += curr_pose.orientation.w


        # for i in msg.poses.position.x:
        #     X_all_value += i
        # for j in msg.poses.position.y:
        #     Y_all_value += j
        # for k in msg.poses.orientation.z:
        #     Orientation_z_all_value += k
        # for l in msg.poses.orientation.w:
        #     Orientation_w_all_value += l

        X_average = X_all_value/(len(msg.poses))
        Y_average = Y_all_value/(len(msg.poses))
        Orientation_z = Orientation_z_all_value/(len(msg.poses))
        Orientation_w = Orientation_w_all_value/(len(msg.poses))



        self.starting_Position.pose.position.x = X_average
        self.starting_Position.pose.position.y = Y_average
        self.starting_Position.pose.orientation.z = Orientation_z
        self.starting_Position.pose.orientation.w = Orientation_w


        # print(self.starting_Position.pose)
    ...
    def calc_cspace(self, mapdata, padding):
        ...
        frontier_x = self.initial_pos[0]
        frontier_y = self.initial_pos[1]
...
#deleted the for loop wed 14:19 #uncommented
        ...
        for i in mapdata.data:  
            if (PathPlanner.is_cell_walkable(mapdata,x,y) == False):
                for X_padding in range(-padding, padding):
                    for Y_padding in range(-padding, padding):
            ...
            elif (PathPlanner.is_cell_unknown(mapdata,x,y) == True):
                for X_frontier_padding in range(-frontier_padding, 1+frontier_padding):
                    for Y_frontier_padding in range(-frontier_padding, 1+frontier_padding):
                          if x+X_frontier_padding in range(frontier_x - 50 ,frontier_x + 50) and y+Y_frontier_padding in range( frontier_y - 50 , frontier_y + 50):
...
#=================================================
    ...
    def plan_path(self, msg):
        ...
        # start = PathPlanner.world_to_grid(mapdata, self.starting_Position.pose.position)
